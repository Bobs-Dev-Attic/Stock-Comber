...
### Changed
- **Older Analysis-history rows now open a fresh analysis instead of the editor.** v0.99.0 reopened
  the stored read-out for rows saved with it, but rows recorded before that (summary-only, no stored
  analysis) fell back to the holdings editor. They now run a fresh analysis of that portfolio and
  show it, so clicking any history row always opens an analysis.
...
### Changed
- **Portfolio: clicking an Analysis-history row opens the Edit dialog with the holdings.** Each row
  in the Portfolio tab's Analysis history is now clickable and opens the holdings editor for that
  portfolio (previously the rows were inert).
...
### Added
- **Export a fund analysis as JSON too.** Alongside the ⭳ CSV button, the Fund X-ray read-out now
  offers **⭳ JSON** — the complete analysis object (scores, coverage, concentration, sectors,
  holdings, suggestions) as `stock-comber-fund-<symbol>-<date>.json`.

## [0.92.0] - 2026-08-24

### Added
- **Export a fund analysis as CSV.** The Fund X-ray read-out gains a **⭳ CSV** button that
  downloads the analysis — a fund-level summary block (score, grade, quality, diversification,
  coverage, concentration, top sector) followed by the full holdings table (ticker, weight,
  sector, score, pass/fail, valuation, covered). Works on a fresh analysis or a reopened saved one.
...
### Changed
- **Buy-below / sell-above now show the % from the current price.** Each target in the
  Portfolio analysis table displays its signed distance from today's price (e.g.
  `$190.00 / −5.0%`, `$220.00 / +10.0%`), so the gap to act on reads at a glance.

## [0.86.0] - 2026-08-24

### Added
- **Ticker autocomplete in the Portfolio Edit dialog.** Each holding's ticker field now
  suggests matching symbols (name + ticker) as you type, backed by `/api/tickers` — the same
  source as Manual search. Arrow keys / Enter or a click picks a suggestion; Escape dismisses
  the list without closing the dialog.
...
### Added
- **Saved column layouts.** A new **▦ Layouts** menu (next to Columns) saves the current set
  and order of visible columns under a name and re-applies it in one click — the column
  equivalent of the filter presets. Stored in `localStorage` (`sc_colpresets`).

### Changed
- **The top pager now sits on the Columns / Export toolbar row.** The ← Prev / Next → control
  is inline with the Columns, Layouts and Export menus instead of on its own line above the
  table.
...
### Added
- **Re-run every ticker currently on a given strategy.** `analyze-queue` gains a
  `--reseed-strategy <name>` option (and the **Analysis queue worker** workflow a
  matching `reseed_strategy` dispatch input) that enqueues every ticker with a stored
  result under that strategy — e.g. `custom` — for a fresh full analysis. Combined
  with v0.69.0, a re-analysed custom-job ticker now regains its real strategy rows
  (the six built-in lenses) plus a refreshed `custom` row. New
  `Storage.tickers_with_strategy(strategy, limit)`.
...
### Changed
- **Scheduled-report dialog layout polish.** The **Enabled** checkbox is now a
  pill **toggle switch**, and **Frequency / Every / Time (Minute past the hour) /
  Days** are arranged in a **responsive 2-column grid** that collapses to a single
  column on narrow/mobile screens (≤520px). The "Every" cell still shows only in
  interval mode.
...
### Fixed
- **History tab now refreshes every time it's opened.** It used to load once per page session and then
  serve that first snapshot (`histLoaded` guard), so runs/jobs/searches created *after* the first view
  didn't appear until you hit ↻ Refresh or reloaded — e.g. today's nightly run or a just-queued
  analysis looked missing. Switching to the History tab now always re-fetches. (The live queue poll
  while something is running is unchanged.)

[0.50.2]: https://github.com/Bobs-Dev-Attic/Stock-Comber/releases/tag/v0.50.2
