"""Bundled stock-index constituent snapshots.

Periodic snapshots for the universe-template feature — an educational
convenience, not a real-time index feed. Constituents drift over time; refresh
these lists when needed. The S&P 500 map carries GICS sector + sub-industry;
Dow 30 and Nasdaq-100 are ticker lists whose sector/industry are looked up from
the S&P map when the name is also in the S&P 500, else filled by enrichment.
"""
...
SP500: dict[str, tuple[str, str]] = {
    ...
    'APP': ('Communication Services', 'Advertising'),
    ...
    'DD': ('Industrials', 'Industrial Conglomerates'),
    ...
    'FERG': ('Industrials', 'Building Products'),
    ...
    'GNRC': ('Industrials', 'Heavy Electrical Equipment'),
    ...
    'RDDT': ('Communication Services', 'Interactive Media & Services'),
    ...
    'VMRK': ('Real Estate', 'Multi-Family Residential REITs'),
...
DOW30: list[str] = ['AAPL', 'AMGN', 'AMZN', 'AXP', 'BA', 'CAT', 'CRM', 'CSCO', 'CVX', 'DIS', 'GS', 'HD', 'HON', 'IBM', 'JNJ', 'JPM', 'KO', 'MCD', 'MMM', 'MRK', 'MSFT', 'NKE', 'NVDA', 'PG', 'SHW', 'TRV', 'UNH', 'V', 'VZ', 'WMT']

NASDAQ100: list[str] = ['AAPL', 'ABNB', 'ADBE', 'ADI', 'ADP', 'ADSK', 'AEP', 'AMAT', 'AMD', 'AMGN', 'AMZN', 'ANSS', 'APP', 'ARM', 'ASML', 'AVGO', 'AZN', 'BKNG', 'BKR', 'CCEP', 'CDNS', 'CDW', 'CEG', 'CHTR', 'CMCSA', 'COST', 'CPRT', 'CRWD', 'CSCO', 'CSGP', 'CSX', 'CTAS', 'CTSH', 'DASH', 'DDOG', 'DXCM', 'EA', 'EXC', 'FANG', 'FAST', 'FTNT', 'GEHC', 'GFS', 'GILD', 'GOOG', 'GOOGL', 'HON', 'IDXX', 'INTC', 'INTU', 'ISRG', 'KDP', 'KHC', 'KLAC', 'LIN', 'LRCX', 'LULU', 'MAR', 'MCHP', 'MDLZ', 'MELI', 'META', 'MNST', 'MRVL', 'MSFT', 'MU', 'NFLX', 'NVDA', 'NXPI', 'ODFL', 'ON', 'ORLY', 'PANW', 'PAYX', 'PCAR', 'PDD', 'PEP', 'PLTR', 'PYPL', 'QCOM', 'REGN', 'ROP', 'ROST', 'SBUX', 'SNPS', 'TEAM', 'TMUS', 'TSLA', 'TTD', 'TTWO', 'TXN', 'VRSK', 'VRTX', 'WBD', 'WDAY', 'XEL', 'ZS']

INDEXES = {
    "sp500": {"name": "S&P 500", "tickers": list(SP500.keys())},
    "dow": {"name": "Dow 30", "tickers": DOW30},
    "nasdaq100": {"name": "Nasdaq-100", "tickers": NASDAQ100},
}
...
def index_rows(key: str) -> list[tuple[str, str, str]]:
    """Return [(ticker, sector, industry)] for an index key ("" -> [])."""
    idx = INDEXES.get((key or "").lower())
    if not idx:
        return []
    out = []
    for t in idx["tickers"]:
        sector, industry = SP500.get(t, ("", ""))
        out.append((t, sector, industry))
    return out
